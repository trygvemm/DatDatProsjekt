...
def get_search(search):
    cursor.execute(
    "SELECT Coffee.coffeeName, CoffeeRoastery.name FROM Coffee INNER JOIN CoffeeRoastery ON Coffee.roasteryID = CoffeeRoastery.roasteryID INNER JOIN Post ON Post.coffeeID = Coffee.coffeeID WHERE Post.note LIKE '%:search%' OR Coffee.description = '%:search%'", {'search': search})
    return cursor.fetchall()
